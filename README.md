## lesson 01
- [easy level](https://github.com/Mullla/js-course/tree/lesson01)
Added variables.
***

## lesson 02
- [easy level](https://github.com/Mullla/js-course/tree/lesson02)
Set values to variables.
- [hard level](https://github.com/Mullla/js-course-hard/tree/lesson02)
Added a number, then multiply its numerals. 
***

## lesson 03
- [easy level](https://github.com/Mullla/js-course/tree/lesson03)
**Calculator**: Added questioning user on money, expenses and amount of expenses. 
- [hard level](https://github.com/Mullla/js-course-hard/tree/lesson03)
Added if-else statement, switch case and ternary operations.
***

## lesson 04
- [easy level](https://github.com/Mullla/js-course/tree/lesson04)
**Calculator**: Remade functionality on functions. 
- [hard level](https://github.com/Mullla/js-course-hard/tree/lesson04)
Added function which checks an input value: work with strings.
***

## lesson 05
- [easy level](https://github.com/Mullla/js-course/tree/lesson05)
**Calculator**: Added functionality with if-else and do-while.
- [hard level](https://github.com/Mullla/js-course-hard/tree/lesson05)
Check, if number is prime.
***

## lesson 06
- [easy & hard level](https://github.com/Mullla/guess-game)
A game, where user should guess a number. Hard level has number of guesses. Used: closure, recursive function.
***

## lesson 07
- [easy level](https://github.com/Mullla/js-course/tree/lesson07)
**Calculator**: Added all variables and almost all functions to object properties.
- [hard level](https://github.com/Mullla/js-course-hard/tree/lesson07)
Show on screen today's day in bold font. Used: DOM.
***

## lesson 08
- [easy level](https://github.com/Mullla/js-course/tree/lesson08)
**Calculator**: Added checking for input types.
***

## lesson 09
- [easy level](https://github.com/Mullla/js-course/tree/lesson09)
**Calculator**: Added inputs from DOM.
- [hard level](https://github.com/Mullla/js-course-hard/tree/lesson09)
Show on screen today's day and time. Add zero to time numerals. Used: Date(), setInterval().
***

## lesson 10
- [easy level](https://github.com/Mullla/js-course/tree/lesson10)
Working with books' chapters and background image from JS.
- [hard level](https://codepen.io/mullla/pen/abBmMbv)
*Codepen*: Add from input to list. 
***

## lesson 11
- [easy level](https://github.com/Mullla/js-course/tree/lesson11)
**Calculator**: Added functionality to money calculator. 
- [hard level](https://github.com/Mullla/js-course-hard/tree/lesson11)
**Calculator**: Added regExp functions: checking for only numbers or literals.
***

## lesson 12
- [easy level](https://github.com/Mullla/todo-list)
Todo list with localStorage.
***

## lesson 13
- [easy level](https://github.com/Mullla/js-course/tree/lesson13)
**Calculator**: Added reset() and this. 
- [hard level](https://github.com/Mullla/js-course-hard/tree/lesson11)
Button changes background with random color.
***

## lesson 14
- easy level
  * [class DomElement](https://github.com/Mullla/new-dom-element)
  * **Calculator**: [OOP, rewrite app on classes](https://github.com/Mullla/js-course/tree/lesson14)
- [hard level](https://github.com/Mullla/new-dom-element/tree/lesson14)
Added square with DomElement class. 
***

## lesson 15
- easy level
  * **Calculator**: [rewrite on ES6 classes](https://github.com/Mullla/js-course/tree/lesson15)
  * *Codepen*: [extending classes](https://codepen.io/mullla/pen/OJbmvqj)
- [hard level](https://github.com/Mullla/js-course-hard/tree/lesson15)
**Calculator**: Combined two similar methods in one.
***

## lesson 16
- easy level
  * **Calculator**: [Added deposit](https://github.com/Mullla/js-course/tree/lesson16)
  * [destructure](https://codepen.io/mullla/pen/bGBRajx)
  *Codepen*: Changed original array.
- hard level
  * [destructure](https://codepen.io/mullla/pen/oNYwVBm)
  *Codepen*: Didn't change original array.
  * [cookies and localStorage](https://github.com/Mullla/js-course-hard/tree/lesson16)
***

## lesson 17
- *Codepen*: [master this](https://codepen.io/mullla/pen/MWboMjo)
- *Codepen*: [master arrays](https://codepen.io/mullla/pen/qBqjzwz)
- *Codepen*: [master selects](https://codepen.io/mullla/pen/JjbJgPQ)
***

## lesson 18
- [easy level](https://github.com/Mullla/js-course/tree/lesson18)
  * **3D Glo**: Added timer
  * master new Date()
- [hard level#1](https://github.com/Mullla/js-course-hard/tree/lesson18_part2)
Throttle and debounce.
- hard level#2
  * *Codepen*: [puzzle with timeouts](https://codepen.io/mullla/pen/poNpRqg)
  * [requestAnimationFrame](https://github.com/Mullla/js-course-hard/tree/lesson18)
***

## lesson 19
- [easy level](https://github.com/Mullla/js-course/tree/lesson19)
**3D Glo**: Modal, menu, animation. 
- [hard level](https://github.com/Mullla/js-course-hard/tree/lesson19)
Smooth scroll.
***

## lesson 20
- [easy level](https://github.com/Mullla/js-course/tree/lesson20)
**3D Glo**: Tabs, events delegation. 
- [hard level](https://github.com/Mullla/js-course-hard/tree/lesson20)
Toggle menu with only 1 delegation.
***

## lesson 21
- [easy level](https://github.com/Mullla/js-course/tree/lesson21)
Slider.
***

## lesson 22
- [easy level](https://github.com/Mullla/js-course/tree/lesson22)
Rewrite todo as Class. Added handler(), deleteItem(), completeItem().
- [hard level](https://github.com/Mullla/js-course-hard/tree/lesson22)
Added animation, editItem().
***

## lesson 23
- [easy level](https://github.com/Mullla/js-course/tree/lesson23)
**3D Glo**: Check inputs' values with regExps. 
- [hard level](https://github.com/Mullla/js-course/tree/lesson23)
Master regular expressions.
***

## lesson 24
- [easy level](https://github.com/Mullla/js-course/tree/lesson24)
**3D Glo**: Added calculator. 
- [hard level](https://github.com/Mullla/js-course/tree/lesson24)
**3D Glo**: Added animation to calculator result.
***

## lesson 25
- [easy level](https://github.com/Mullla/js-course/tree/lesson25)
**3D Glo**: Validator plugin. Used: OOP. 
***

## lesson 26
- [easy level](https://github.com/Mullla/js-course/tree/lesson26)
**3D Glo**: Ajax sending. 
- [hard level](https://github.com/Mullla/js-course/tree/lesson26)
**3D Glo**: Added preloader to loading message.
***

## lesson 27
- [easy level](https://github.com/Mullla/js-course/tree/lesson27)
Added comments to try-catch project.
***

## lesson 28
- [easy level](https://github.com/Mullla/js-course/tree/lesson28)
**3D Glo**: Rewrite ajax sending with promises. 
- [hard level](https://github.com/Mullla/js-course/tree/lesson28)
Get data from json with promises.
***

## Bonus lesson
- [auth](https://github.com/Mullla/auth-lesson)
 * part#1: Add every user as an object to array, using prompts. Render list of users to the page. Add information to localStorage.
 * part#2: Delete user from database and localStorage. Add authorization with login and password using alert.
- [heroes](https://github.com/Mullla/bonus-heroes)
Render heroes cards and get information from json-file with ajax.
